...
% fontspec 英文字體設定
\defaultfontfeatures{
    AutoFakeBold=2.5, % 沒有給定粗體時使用偽粗體粗度 2.5
    AutoFakeSlant=0.2,  % 沒有給定斜體時使用偽斜體斜度 0.2
}
...
\geometry{
    paper=a4paper,
    % includehead=false, % 將 headheight headsep 計入 top
    % headheight=1cm,
    % headsep=1cm,
    top=4cm,
    includefoot,
    ...
    \begin{titlepage}
        \begin{singlespace}
            \begin{center}
                ...
                \vspace{5mm}
                \ifthenelse{\equal{\thesisIsDraft}{Y}}{
                    \fontsize{24pt}{0pt}{\thesisDegreeZHTW 論文初稿}
                }{
                ...
                % 題目
                \setstretch{1.5}\selectfont % 暫時將中文題目設為 1.5 倍行高
...
\newenvironment{thesisAbstractEnvZHTW}{
    \phantomsection % 目次超連結錨點
    \addcontentsline{toc}{chapter}{\thesisAbstractTitleZHTW}  % 加入目次

    {\setstretch{1.0} \chapter*{\thesisTitleZHTW}} % 暫時修改題目行高
    % \vspace{5mm}
...
\newenvironment{thesisAbstractEnvEN}{
    \phantomsection % 目次超連結錨點
    \addcontentsline{toc}{chapter}{\thesisAbstractTitleEN} % 加入目次

    {\setstretch{1.0} \chapter*{\thesisTitleEN}} % 暫時修改題目行高
    % \vspace{5mm}
...
% 致謝指令
\newcommand{\thesisAddAcknowledgement}{
    \chapter*{\thesisAcknowledgementTitle}

    \phantomsection % 目次超連結錨點
    \addcontentsline{toc}{chapter}{\thesisAcknowledgementTitle} % 加入目次
}
...
\newenvironment{thesisDenotationEnv}[1][2.5cm]{
    ...
    \phantomsection % 目次超連結錨點
    \addcontentsline{toc}{chapter}{\thesisDenotationTitle} % 加入目次
...
% 設定目次標題樣式
\renewcommand{\contentsname}{\centerline{\normalfont\thesisTableOfContentsTitle}} % 設定目次標題名
\setlength{\cftbeforetoctitleskip}{-1cm} % 抵銷目次標題上方的額外間距

% 設定目次樣式
\setcounter{tocdepth}{2} % 設定目次深度至 2 => 子節
\setcounter{secnumdepth}{2} % 設定編號深度 2 => 子節

\titlecontents{chapter}[0em]
{}
{\thesisChapterTitleTocFormat} % 須改中英顯示間距於 config
{}
...
%　設定表目次標題
\renewcommand{\listtablename}{\centerline{\normalfont\thesisListOfTablesTitle}} % 設定表目次標題名
\setlength{\cftbeforelottitleskip}{-1cm} % 消除表目次標題上方的額外間距

% 設定表目次樣式
\setlength{\cftbeforetabskip}{0pt} % 消除表目次章間間距
\renewcommand{\tablename}{\thesisTableName} % 設定表目次頁標號，如 表 1, 表 2 .... 
...
% 設定圖目次標題
\renewcommand{\listfigurename}{\centerline{\normalfont\thesisListOfFiguresTitle}} % 設定圖目次標題名
\setlength{\cftbeforeloftitleskip}{-1cm} % 消除圖目次標題上方的額外間距

% 設定圖目次樣式
\setlength{\cftbeforefigskip}{0pt} % 消除圖目次章間間距
\renewcommand{\figurename}{\thesisFigureName} % 設定圖目次頁標號，如 圖 1, 圖 2 ....
...
\newcommand{\thesisMakeListOfTables}{
    ...
    \renewcommand{\addvspace}[1]{} % 暫時讓 addvspace 指令失效
...
% - after-sep: 標題下方的空白間距
% - right（可選）: 標題右邊的縮排（一般很少用）。
% --------------------------------------------------

% 設定章標題編號方式
\renewcommand{\thechapter}{\thesisChapterTitleNumbering{chapter}}
...
\titlespacing*{\chapter}{0pt}{-1cm}{12pt} % 設定章標題下方留空一行

% 設定節標題編號方式
\renewcommand{\thesection}{\thesisSectionTitleNumbering{chapter}{section}}
...
\newcommand{\thesisAppendixMatter}{
    ...
    % 設定章標題編號方式
    \renewcommand{\thechapter}{\thesisAppendixChapterTitleNumbering{chapter}}

    % 將附錄節編號設回數字
    \renewcommand{\thesection}{\thesisAppendixSectionTitleNumbering{chapter}{section}}
...
% enumitem 列表設定
\setlist[1]{ % 第一層級列表.
    listparindent=\parindent, % 設定首行縮排
    labelindent=\parindent, % 從標籤開始縮排
    leftmargin=*,
}

\setlist[2]{ % 第二層級列表
    listparindent=\parindent, % 設定內容首行縮排
    labelindent=\parindent, % 從標籤開始縮排
    leftmargin=*,
}
...
\setlist[enumerate, 1]{ % 第一層有序列表
    label=\arabic*., % 樣式為 1.
}

\setlist[enumerate, 2]{ % 第二層有序列表
    label=(\arabic*), % 樣式為 (1)
}

\setlist[enumerate, 3]{ % 第三層有序列表
    label=\alph*., % 樣式為 a.
}
...
\hypersetup{
    colorlinks=true,
    linkcolor=black, % 章節、圖表等內部連結顏色
    filecolor=black,
    anchorcolor=black,
    citecolor=black, % 引用文獻連結顏色
    urlcolor=black, % 所有網址連結顏色
}
...
\AtEveryCitekey{%
    \ifciteseen
    {}%
    {\defcounter{maxnames}{99}}% 第一次引用時不限制作者數量（可選）
}

% 設定中外文參考文獻篩選標準
\defbibcheck{thesisRefZh}{%
    \ifboolexpr{
        test {\iffieldequalstr{langid}{chinese}}
        or
        test {\iffieldequalstr{langid}{zh}}
        or
        test{\iffieldequalstr{langid}{chinese-traditional}}
    }
    {}{\skipentry}%
}

\defbibcheck{thesisRefOtherLang}{%
    \ifboolexpr{
        not(
        test {\iffieldequalstr{langid}{chinese}}
        or
...
\newcommand{\thesisMakeReference}{
    ...
    \phantomsection % 目次超連結錨點
    \addcontentsline{toc}{chapter}{\thesisBibliographyTitle} % 加入目次

    % 插入參考文獻
    \ifthenelse{\equal{\thesisSplitRefByLang}{Y}}{
        \section*{中文文獻}
        \printbibliography[
            check=thesisRefZh,
            heading=none,
